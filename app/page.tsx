...
export default function Page() {
    ...
    const result = useMemo<BlendResult>(() => {
        ...
        // If additive ethanol strength equals target, it can't move mixture toward target unless already there.
        if (Math.abs(Ea - Et) < 1e-12) {
            if (Math.abs(Eg - Et) < 1e-12) {
                return { ok: true, Ve: 0, Vt: Vg, Efinal: Et, waterL: 0, note: 'Sei già al target.' };
            }
            return { ok: false, errors: ['Impossibile: la % del bioetanolo è uguale al target.'] };
        }

        // Solve: (Vg*Eg + Ve*Ea) / (Vg + Ve) = Et  ->  Ve = Vg*(Et - Eg)/(Ea - Et)
        ...
        if (VeRaw < -1e-9) {
            return {
                ok: false,
                errors: [
                    'Risultato negativo: la benzina di partenza è già sopra il target (o il verso non consente di arrivarci aggiungendo questo bioetanolo).',
                ],
            };
        }

        // Unreachable if target > additive strength while base < target
        ...
        // Water introduced by ethanol source (assuming remainder is water/impurities)
            ...
            <div className="mx-auto max-w-3xl space-y-4">
                <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                        <Button variant="secondary" onClick={() => quickPreset('yourCase')}>
                            Preset 13L E6 → E40 con 96%
                        </Button>
                        <Button variant="outline" onClick={() => quickPreset('e10')}>
                            Benzina E10
                        </Button>
                        <Button variant="outline" onClick={() => quickPreset('e0')}>
                            Benzina E0
                        </Button>
                    </div>
                </div>
                ...
                <Card className="rounded-2xl shadow-sm">
                    <CardContent className="p-4 md:p-6">
                        ...
                        <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-gray-600">
                            ...
                            <li>
                                Se la benzina di partenza è già sopra il target, il calcolo richiede “togliere” etanolo (non possibile
                                aggiungendone).
                            </li>
                            <li>Se vuoi aggiungere anche benzina dopo (per arrivare a un volume preciso), si può aggiungere quella modalità.</li>
                        </ul>
                    </CardContent>
                </Card>
            </div>
